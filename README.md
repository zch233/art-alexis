...
2026-09-20 源码还原主题1.2.0：以客户导出的 site-package 首页及详情页为基准，替代1.1.0的近似布局。验证见 `docs/source-parity-review.md`，更新步骤见 `deploy-managed/THEME-UPDATE.md`。只改本地主题，线上不自动发布。

GoDaddy 托管版单独入口：`deploy-managed/README.md`。与下面 VPS 方案并存，共用业务代码；安装包输出到被 Git 忽略的 `deploy-managed/dist/`。不要把 VPS 数据库恢复脚本用于托管环境。
...
完整站点启动：`npm ci` → `npm run bu ild` → `npm run dev:wp`，打开 http://127.0.0.1:9400 。后台 /wp-admin/；登录资料在 work/local-access.json，运行数据保存在 work/runtime，禁止公开提交或部署。
