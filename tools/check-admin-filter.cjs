...
const rows=()=>p.locator('#the-list tr.type-aa_artwork');
const count=async()=>await rows().count()?Number((await p.locator('.tablenav.top .displaying-num').textContent()).replace(/[^0-9]/g,'')):0;
const all=await count();assert(all>0);
const expected={};while(true){for(const name of await rows().locator('.column-aa_state').allTextContents())expected[name.trim()]=(expected[name.trim()]||0)+1;const next=p.locator('.tablenav.top a.next-page');if(!await next.count())break;await p.goto(await next.getAttribute('href'))}await p.goto(base+'/wp-admin/edit.php?post_type=aa_artwork');
...
for(const option of options){await p.locator('#aa-filter').selectOption(option.value);await Promise.all([p.waitForNavigation(),p.locator('#post-query-submit').click()]);assert.equal(await count(),expected[option.label]||0);assert.equal(new URL(p.url()).searchParams.get('aa_filter_collection'),option.value);assert(!new URL(p.url()).searchParams.has('aa_collection'));checks.push(option.label);}
await p.locator('#aa-filter').selectOption('');await Promise.all([p.waitForNavigation(),p.locator('#post-query-submit').click()]);assert.equal(await count(),all);checks.push('reset all');
...
await p.goto(url+'&orderby=title&order=asc');assert.equal(await count(),expected[chosen.label]);checks.push('sort preserves filter');
